...
use super::envelope::{Envelope, EnvelopeParams, EnvelopeState};
use super::filter::{BiquadFilter, FilterPreset};
...
pub struct Voice {
    ...
    pub filter:     BiquadFilter,   // ← NEW in Step 5
...
impl VoicePool {
    ...
    fn steal_voice(&self) -> usize {
        if let Some(idx) = self.voices.iter().position(|v| {
            v.active && v.envelope.state() == EnvelopeState::Release
        }) {
            return idx;
        }
        self.voices
