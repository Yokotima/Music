...
pub struct BiquadFilter {
    /// Current (active) coefficients — may be mid-interpolation
    current: BiquadCoeffs,
    /// Target coefficients after a parameter change
    target:  BiquadCoeffs,
    /// Delay line: x[n-1], x[n-2] (input history)
    x1: f32, x2: f32,
    /// Delay line: y[n-1], y[n-2] (output history)
    y1: f32, y2: f32,
    /// Samples remaining in the current interpolation
    smooth_remaining: u32,
    /// Cached parameters for `set_params` deduplication
...
impl BiquadFilter {
    ...
    pub fn set_params(&mut self, filter_type: FilterType, cutoff_hz: f32, q: f32) {
        ...
        if filter_type == self.filter_type
            && (cutoff_hz - self.cutoff_hz).abs() < 0.01
            && (q         - self.q        ).abs() < 0.001
        {
            return;
        }
...
// ─────────────────────────────────────────────────────────────────────────────
// FilterPreset — ready-to-use configs for each instrument (Step 7)
// ─────────────────────────────────────────────────────────────────────────────

/// A named filter configuration for an instrument preset.
#[derive(Debug, Clone, Copy)]
